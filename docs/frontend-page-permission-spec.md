...
- 截至 `2026-04-08`，`frontend/` 中已完成前端页面骨架、布局、权限控制、导航与后台页面原型
- 当前完成状态基于 `Mock` 服务层与本地可构建预览，真实 `Django/DRF` 接口接入待后续推进
- 截至 `2026-04-08`，本轮已补齐以下落地项：
  - [x] 统一页内反馈提示组件与状态块
  - [x] 补齐分类页、标签页、RSS、我的下载、我的发布等前台页面的 `loading / empty / error`
  - [x] 补齐后台首页、用户、资源、分类、标签、公告、XBT 同步、审计日志、系统设置等管理页状态处理
  - [x] 统一角色、状态、公告状态与可见范围徽标展示
  - [x] 完善移动端前台导航与后台侧边栏收起交互
  - [x] 补齐首页、资源列表、分类页、标签页与我的下载页的直接下载 / 重新下载短路径动作

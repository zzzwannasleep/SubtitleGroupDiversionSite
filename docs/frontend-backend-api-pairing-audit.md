...
- **但还不属于“已完成真实 API 联调、可直接移除默认 mock”**状态
...
- **暂时不要去掉前端默认 `mock`**
- 必须等下面列出的阻塞项都补齐并联调验证后，再考虑切到默认真实 API
...
## 5. 已实现但当前未被 SPA 使用的兼容接口

这类接口后端存在，但当前前端已经有其它主路径可用，因此优先级低于第 3 节。
...
当前前端仍然保留 `mock` 回退逻辑，仓库文档也明确写着“真实 `Django/DRF` 接口接入待后续推进”。在这种状态下：

- 现在**不要去掉前端默认 `mock`**
- 第 3 节中原本的阻塞 API 已补齐
- 下一步应做一轮真实 API 联调
- 联调完成后，再决定是否把默认数据源从 `mock` 改成 `api`

## 8. 建议的收口顺序

原建议顺序中的前端补齐项已完成。当前建议收口为：

- 做一轮真实 `Django/DRF` API 联调
- 联调完成后，评估是否把默认数据源从 `mock` 改成 `api`

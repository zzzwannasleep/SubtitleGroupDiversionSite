...
- [ ] 实现 `/403` 页面
- [ ] 实现 `/404` 页面
- [ ] 实现统一的 `loading / empty / error` 状态组件
- [ ] 实现登录态初始化与 `/auth/me` 同步
...
- [ ] 设计主站与 `XBT` 的同步策略
- [ ] 实现用户 `passkey` / `torrent_pass` 生成规则
- [ ] 实现 `xbt_users` 同步
- [ ] 实现 `xbt_files` 白名单同步
- [ ] 实现禁用用户状态同步
- [ ] 实现资源发布后自动同步到 `XBT`
- [ ] 实现 `passkey` 重置后的 `XBT` 同步
- [ ] 实现同步失败重试与日志
